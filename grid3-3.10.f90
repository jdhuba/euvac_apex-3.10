...
!     loop over longitude (phi direction)
    ...
    !        print *,k,taskid,kk,blon0
        ...
        !$$$            nzg    = nz1
        !$$$          endif
        !$$$          if ( j .gt. nf1 .and. j .le. nf2 ) then
        !$$$            gamss  = gams2
        !$$$            gamssg = gams2m
        !$$$            nzg    = nz2
        !$$$          endif
        !$$$          if ( j .gt. nf2 .and. j .le. nf3 ) then
        !$$$            gamss  = gams3
        !$$$            gamssg = gams3m
        !$$$            nzg    = nz3
        !$$$          endif
...
!            nzg    = nz1
        ...
        !          f00  = gamss  ! needs to be in namelist_mod

            do i=1,nz0+1
                farg  = float(i-nz0h)
                darg  = float(nz0) / 10.
                f00(i) = gamss + (.99-gamss) * &
            !            f00(i) = gamss + .95 *
                exp(-(farg/darg)**2)
            !            if(taskid.eq.1 .and. j.eq.nf/2 .and. k.eq.nl/2) then
            !              print *,'f00',i,f00(i)
...
!      if ( taskid .eq. 2 ) then
!        do i = 1,nz
!          print *,'i,d22s,dels',i,d22s(i,nf/2,nl/2),dels(i,nf/2,nl/2)
!        enddo
!      endif
...
!!$                ax1 = x(i,j,k+1) - x(i,j+1,k)
!!$                ay1 = y(i,j,k+1) - y(i,j+1,k)
!!$                az1 = z(i,j,k+1) - z(i,j+1,k)
!!$                            
...
!!$                            
!!$                bx1 = x(i,j,k+1) - x(i+1,j,k)
!!$                by1 = y(i,j,k+1) - y(i+1,j,k)
!!$                bz1 = z(i,j,k+1) - z(i+1,j,k)
...
! original
                ...
                ax1 = x(i+1,j,k) - x(i,j+1,k)
                ay1 = y(i+1,j,k) - y(i,j+1,k)
                az1 = z(i+1,j,k) - z(i,j+1,k)

                bx1 = x(i+1,j+1,k) - x(i,j,k)
                by1 = y(i+1,j+1,k) - y(i,j,k)
                bz1 = z(i+1,j+1,k) - z(i,j,k)
...
!     to obtain blatss, delph, delhp, bmpf
    ...
    do k = 1,nlp1
        do j = 1,nfp1
            ...
            if (k .eq. nlp1) bmtmp(i,j,k) = bmtmp(i,j,1)
                xp(i,j,k) = r * cos( blat * po180 ) * sin( blon * po180 )
                yp(i,j,k) = r * sin( blat * po180 )
                zp(i,j,k) = r * cos( blat * po180 ) * cos( blon * po180 )
                if ( i == nzh) then
