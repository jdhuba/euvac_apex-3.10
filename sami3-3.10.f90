...
! only need these on the master
    ...
    if(taskid == 0) then
        open ( unit=10, file='sami3-3.10.namelist'  )
...
!******************** worker task *******************************************
    ...
    ! parallel transport
        ...
        iday  = ceiling( (hrut+1.e-6) /23.999)
...
! Some local variables
    ...
    namelist / go / maxstep,hrmax,dthr,hrpr,dt0, &
      ...
      lhwm93,lhwm14,anu_drag0,kp,xhardy
...
! send the namelist data to all the other processors
    ...
    call mpi_bcast(fbar,7,MPI_REAL,0,MPI_COMM_WORLD,ierr)
    call mpi_bcast(f10p7,7,MPI_REAL,0,MPI_COMM_WORLD,ierr)
    call mpi_bcast(ap,7,MPI_REAL,0,MPI_COMM_WORLD,ierr)
    call mpi_bcast(iyear,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
    call mpi_bcast(nday,7,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
...
!    call mpi_bcast(plon,1,MPI_REAL,0,MPI_COMM_WORLD,ierr)
    ...
    call mpi_bcast(kp,7,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
...
!!$            flux(i) = f74 * ( 1. + ai * ( p - 80.) ) * 1.e9
    ...
    call mpi_bcast(fluxdat,linesuv*2, &
    MPI_REAL,0,MPI_COMM_WORLD,ierr)
...
!!$    if ( hrut == 0 .or. tneut >= 0.25 ) &
!!$    call hardy_sami3 ( hrut,nfl,nll                  ) ! calculates precip stuff every 15 min

!    print *,'max ..',maxval(preciprn),maxval(preciprs)

! old stuff from glenn sami3_lfm
! don't think correct

!!$    denntotn = 0.92 * denn(iin(nfl,nll),nfl,nll,ptn2) + &
!!$               1.00 * denn(iin(nfl,nll),nfl,nll,pto2) + &
!!$               0.56 * denn(iin(nfl,nll),nfl,nll,pto)
!!$    denntots = 0.92 * denn(iis(nfl,nll),nfl,nll,ptn2) + &
!!$               1.00 * denn(iis(nfl,nll),nfl,nll,pto2) + &
!!$               0.56 * denn(iis(nfl,nll),nfl,nll,pto)
!!$
!!$!   N2,O2,0
!!$
!!$    do i = 1,nz
!!$      pnout(i,ptn2) = preciprn(iin(nfl,nll),nfl,nll) * &
!!$                      0.92 * denn(iin(nfl,nll),nfl,nll,ptn2) / denntotn
!!$      psout(i,ptn2) = preciprs(iis(nfl,nll),nfl,nll) * &
!!$                      0.92 * denn(iis(nfl,nll),nfl,nll,ptn2) / denntots
!!$      pnout(i,pto2) = preciprn(iin(nfl,nll),nfl,nll) * &
!!$                      1.00 * denn(iin(nfl,nll),nfl,nll,pto2) / denntotn
!!$      psout(i,pto2) = preciprs(iis(nfl,nll),nfl,nll) * &
!!$                      1.00 * denn(iis(nfl,nll),nfl,nll,ptow) / denntots
!!$      pnout(i,pto)  = preciprn(iin(nfl,nll),nfl,nll) * &
!!$                      0.56 * denn(iin(nfl,nll),nfl,nll,pto)  / denntotn
!!$      psout(i,pto)  = preciprs(iis(nfl,nll),nfl,nll) * &
!!$                      0.56 * denn(iis(nfl,nll),nfl,nll,pto)  / denntots
!!$    enddo
...
!!$      u2(i,nfl,nll) = pnout(i,ptn2)
!!$      u3(i,nfl,nll) = pnout(i,pto2)
!!$      u4(i,nfl,nll) = pnout(i,pto)

    enddo

    do i = 1,nz
        do j = nion1,nion2
!          pnout(i,j)     =  preciprn(i,nfl,nll)   &
!                                 * denn(i,nfl,nll,j) /  denntotn
!          psout(i,j)     =  preciprs(i,nfl,nll)   &
!                                 * denn(i,nfl,nll,j) /  denntots
